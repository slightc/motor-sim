...
上板验证不是"转起来了"就算完，要**结构化、可回归、能反哺 core**。流程：

1. **前置门槛**：`bash test/run_host_test.sh` 全绿 + `pio run` 通过，才上板（主机回归是真机的基线）。
2. **填报告**：复制 `docs/validation_report_template.md`，按模板逐项记录——元信息(可复现) /
   上电安全 / 自整定 Rs·Ld·Lq / 开环 I/f / 有感闭环 / **无感闭环** / 仿真↔实测对齐 / 结论。
...
> 即 `MODE_PARAM_ID → MODE_SENSORLESS` 端到端是否在真机闭合。
