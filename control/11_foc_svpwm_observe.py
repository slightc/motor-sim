...
T_END = 0.30
T_STEP = 0.15                   # 速度阶跃时刻：30 -> 50 rad/s
T_LOAD = 0.10                   # 恒定负载转矩 (N·m)
...
print(f"速度阶跃: 30 -> 50 rad/s @ {T_STEP}s, 负载={T_LOAD}N·m")
print("-" * 60)
print(f"末速        : {wm_buf[-1]:.2f} rad/s (目标 50)")
print(f"稳态均速    : {wm_buf[ss].mean():.2f} rad/s, 速度纹波 ±{wm_buf[ss].std():.3f}")
print(f"稳态 i_q    : {iqt[ss].mean():.3f} A (参考 {iqr[ss].mean():.3f})")
print(f"稳态 i_d    : {idt[ss].mean():.3f} A (参考 {idr[ss].mean():.3f}, <0=弱磁)")
...
fig.suptitle("Sensored FOC with SVPWM Inverter - Waveform Observation", fontsize=15, fontweight="bold")

# 选一个稳态电气周期窗口用于细节观测（电频率 = p*wm/2pi ≈ 4*50/2pi ≈ 31.8Hz, 周期 ~31ms）
w_el = cfg.electrical.p * 50.0                           # 电角速度 rad/s
T_el = 2 * math.pi / w_el                                # 电气周期 ~31.4ms
...
ax.plot(t_buf, sp_buf, "k--", lw=1.2, label="setpoint")
ax.plot(t_buf, wm_buf, "b", lw=1.0, label="omega_m")
ax.set_title("Motor State: Speed Tracking"); ax.set_xlabel("t [s]"); ax.set_ylabel("omega_m [rad/s]")
ax.legend(loc="lower right"); ax.grid(alpha=0.3)

ax = plt.subplot(3, 2, 2)                                # ② 转矩
ax.plot(t_buf, tq_buf, "g", lw=0.8)
ax.axhline(T_LOAD, color="r", ls="--", lw=1.0, label=f"load {T_LOAD} N·m")
...
ax.plot(t_buf[zoom]*1e3, ia[zoom], "r", lw=1.0, label="i_a")
ax.plot(t_buf[zoom]*1e3, ib[zoom], "g", lw=1.0, label="i_b")
ax.plot(t_buf[zoom]*1e3, ic[zoom], "b", lw=1.0, label="i_c")
...
ax = plt.subplot(3, 2, 4)                                # ④ dq 电流(真值 vs 参考)
ax.plot(t_buf, iqt, "b", lw=0.9, label="i_q (true)")
ax.plot(t_buf, iqr, "c--", lw=1.2, label="i_q ref")
ax.plot(t_buf, idt, "r", lw=0.9, label="i_d (true)")
ax.plot(t_buf, idr, "m--", lw=1.2, label="i_d ref (<0: field weakening)")
ax.set_title("Current: dq-axis (true vs reference)")
ax.set_xlabel("t [s]"); ax.set_ylabel("i [A]"); ax.legend(loc="upper right", ncol=2); ax.grid(alpha=0.3)

ax = plt.subplot(3, 2, 5)                                # ⑤ SVPWM 调制马鞍波(稳态周期窗)
ax.plot(t_buf[zoom]*1e3, vma[zoom], "r", lw=1.0, label="v_a* (mod)")
ax.plot(t_buf[zoom]*1e3, vmb[zoom], "g", lw=1.0, label="v_b* (mod)")
ax.plot(t_buf[zoom]*1e3, vmc[zoom], "b", lw=1.0, label="v_c* (mod)")
...
tp = t_buf[pwm]*1e6
ax.plot(tp, carr_buf[pwm], "0.6", lw=0.8, label="carrier")
ax.plot(tp, vma[pwm], "r", lw=1.2, label="v_a* mod")
ax.step(tp, la[pwm], "k", lw=1.0, where="post", label="leg_a (switch)")
...
out = os.path.abspath(os.path.join(out_dir, "11_foc_svpwm_observe.png"))
